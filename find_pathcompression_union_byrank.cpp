...
int find_withPathCompression(int parent[], int x){
    if(parent[x]==x) return x;
    else{
        parent[x] = find_withPathCompression(parent,parent[x]);
        return parent[x];
    }
}

void Union(int parent[], int rank[], int x, int y){
    int x_rep = find_withPathCompression(parent,x);
    int y_rep = find_withPathCompression(parent,y);
